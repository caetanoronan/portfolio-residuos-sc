        ...
        try:
            ...
            popup = folium.Popup(
                ...
                <div style='font-family: Arial; font-size: 13px; min-width: 240px;'>
                    <h4 style='margin: 0 0 8px 0; color: {cor_por_porte(row['porte'])};'>{row['NM_MUN']}</h4>
                    <div style='background:#e3f2fd;padding:6px;margin:4px 0;border-left:4px solid #1976d2;'>
                        <b>População:</b> {row['populacao']:,.0f}
                    </div>
                    <div style='background:#e8f5e9;padding:6px;margin:4px 0;border-left:4px solid #388e3c;'>
                        <b>Doméstico:</b> {row['domestico_t_ano']:,.0f} t/ano
                    </div>
                    <div style='background:#fff8e1;padding:6px;margin:4px 0;border-left:4px solid #fb8c00;'>
                        <b>Reciclável (10%):</b> {row['reciclavel_t_ano']:,.0f} t/ano
                    </div>
                    <div style='margin-top:6px;color:#555;'>Porte: <b>{row['porte']}</b></div>
                </div>
                """,
                max_width=320
            )
            marker = folium.CircleMarker(
                ...
                fill=True,
                fill_color=cor_por_porte(row['porte']),
                fill_opacity=0.8,
                weight=2
            )
            marker.add_child(popup)
            if row['porte'] == 'Pequeno Porte':
                fg_small.add_child(marker)
            elif row['porte'] == 'Médio Porte':
                fg_medium.add_child(marker)
            else:
                fg_large.add_child(marker)
        except Exception as e:
            continue
